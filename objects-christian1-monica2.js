...
console.log(person.firstName)
console.log(person.lastName)
console.log(`${person.firstName} ${person.lastName} is from plant ${person.homePlanet}`)
//output: Arthur Dent is from plant Earth
...
const product = {
    ...
    totalWithTax: function () {
        const totalTaxes = (product.price * 0.07)
        const totalPrice = (totalTaxes + product.price)
        const totalRounded = (totalPrice.toFixed(2))
        return totalRounded
    }
  }
...
//   let random = ("Waffles", "Fluffy", "Spelunky", "Hank")
//   let randomJoin = random.join("")
//     const animalName = animals.filter((value) => value.name === )
